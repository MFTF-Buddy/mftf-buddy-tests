...
use MFTFBuddy\Tests\Util\ModuleResolver\ModuleResolverService;
...
use MFTFBuddy\Tests\Util\ModuleResolver\AlphabeticSequenceSorter;
use MFTFBuddy\Tests\Util\ModuleResolver\SequenceSorterInterface;
...
class ModuleResolver
{
     ...
     */
    const MB_MODULE_ALLOWLIST = 'MB_MODULE_ALLOWLIST';

    /**
     * Environment field name for module blocklist.
     */
    const MB_MODULE_BLOCKLIST = 'MB_MODULE_BLOCKLIST';

    /**
     * Environment field name for Magento module blocking.
     */
    const MB_MODULE_BLOCK_MAGENTO = 'MB_MODULE_BLOCK_MAGENTO';

    /**
     * Environment field name for custom module paths.
     */
    const MB_CUSTOM_MODULE_PATHS = 'MB_CUSTOM_MODULE_PATHS';
     ...
     *
     * @var \MFTFBuddy\Tests\Config\DataInterface
     */
    ...
    {
        $objectManager = \MFTFBuddy\Tests\ObjectManagerFactory::getObjectManager();
    ...
    {
        $moduleAllowlist = getenv(self::MB_MODULE_ALLOWLIST);
    ...
    private function printMagentoVersionInfo()
    {
        ...
        $url = UrlFormatter::format(getenv('MB_MAGENTO_BASE_URL')) . $this->versionUrl;
    ...
    private function removeBlocklistModules($modulePaths)
    {
        ...
        $moduleBlocklist = $this->getModuleBlocklist();
        $moduleAllowlist = $this->getModuleAllowlist();
        $moduleBlockMagento = (bool)getenv(self::MB_MODULE_BLOCK_MAGENTO);
        foreach ($modulePathsResult as $moduleName => $modulePath) {
            $blockModule = false;
            // Remove module if it is in blocklist
            if (in_array($moduleName, $moduleBlocklist)) {
                $blockModule = true;
            }
            // Remove module if Magento module should be blocked
            if ($moduleBlockMagento && 0 === strpos($moduleName, 'Magento_')) {
                if (!in_array($moduleName, $moduleAllowlist)) {
                    $blockModule = true;
                }
            }
            if ($blockModule) {
    ...
    {
        $moduleBlocklist = getenv(self::MB_MODULE_BLOCKLIST);

        if (empty($moduleBlocklist)) {
        return $this->moduleBlocklist;
    }
        return array_merge(
            $this->moduleBlocklist,
            array_map('trim', explode(',', $moduleBlocklist))
        );
    }
